...
export const InteractiveHero = () => {
  ...
  // 1. Mouse Coordinates (normalized 0-1)
  const mouseX = useMotionValue(0.5);
  const mouseY = useMotionValue(0.5);

  // 2. Smooth Springs for LERP trailing effect
  ...
  const cursorX = useTransform(smoothX, [0, 1], ["0%", "100%"]);
  const cursorY = useTransform(smoothY, [0, 1], ["0%", "100%"]);
  ...
  return (
    <div className="relative w-full h-[85vh] bg-black overflow-hidden flex items-center justify-center perspective-2000">
        ...
        <div className="absolute inset-0 z-30 flex flex-col justify-end p-24 gap-10">
          ...
          <div className="flex items-center gap-12 mt-4">
            ...
            <div className="flex flex-col gap-1">
               ...
               <span className="text-[10px] font-bold tracking-[0.2em] text-white/80 uppercase">Revision: v9.2.5</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="absolute top-32 left-32 flex flex-col gap-1 z-40 pointer-events-none">
          <span className="text-[9px] text-white/10 tracking-[0.5em] font-mono">
